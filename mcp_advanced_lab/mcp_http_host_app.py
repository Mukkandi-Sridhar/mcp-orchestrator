...
class MCPHTTPHostApp(MCPHTTPClient):
   ...
   def __init__(self, server_url: str, roots_dir: str):
       ...
       # In many lab environments, the base_url or API keys are handled via environment variables
           ...
           "function": {
               ...
               "parameters": {
                   ...
                   "properties": {
                       "uri": {
                           "type": "string",
                           "description": "The URI of the resource to read (e.g., file://workspace/test.txt)"
                       }
                   },
                   "required": ["uri"]
               }
           ...
           "function": {
               ...
               "parameters": {
                   ...
                   "properties": {
                       ...
                       "arguments": {
                           "type": "object",
                           "description": "Arguments for the prompt template as a JSON object"
                       }
                   },
                   "required": ["name"]
               }
   ...
   async def execute_tool(self, tool_name: str, arguments: dict):
       ...
       if tool_name == "mcp_get_prompt":
           ...
           try:
               result = await self.get_prompt(name, prompt_args)
       ...
       try:
           ...
           if hasattr(result, 'content') and isinstance(result.content, list) and len(result.content) > 0:
               ...
               text_result = result.text # Fallback for non-standard tool responses
           else:
   ...
   def create_interface(self):
       ...
       async def chat_wrapper(message, chat_history):
           ...
           # Gradio handles history updates in the UI
           # We return the new list of messages
           return chat_history + [
       ...
       with gr.Blocks(title="MCP HTTP AI Host") as interface:
           ...
           **Model:** {self.model}
           ...
           The AI can use all available MCP tools, resources, and prompts.
           """)
...
def main():
   if len(sys.argv) < 3:
       print("Usage: python mcp_host_app.py <server_url> <roots_dir>")
       print("Example: python mcp_host_app.py http://127.0.0.1:8000 workspace")
       sys.exit(1)
